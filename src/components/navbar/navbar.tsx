      ...
      <nav className="flex items-center justify-between flex-wrap bg-white py-3 lg:px-12 shadow border-solid rounded-b-md">
          <div className={`${navHidden ? '' : 'border-b-2 pb-5'} flex justify-between lg:w-auto w-full lg:border-b-0 pl-6 pr-2 border-solid border-gray-300 lg:pb-0`}>
              ...
              <div className="block lg:hidden">
                  <label onClick={() => setNavHidden(!navHidden)} className="flex items-center px-3 py-2 border-2 rounded text-gray-700 border-gray-700 hover:text-gray-700 hover:border-gray-700">
                      <FontAwesomeIcon icon={faBars}/>
                  </label>
              </div>
          </div>

          <div id="menu" className={`${navHidden ? 'hidden' : ''} lg:flex w-full flex-grow flex-col lg:flex-row lg:flex lg:items-center lg:justify-between lg:w-auto lg:px-3 px-8`}>
              ...
              <div className="flex text-gray-800 text-md">
                  ...
                  <Link className="nav-link" href={getClientDashboardRoute()}>
                      {session?.user?.name ? `Hello, ${session?.user?.name ?? 'NULL'}` : 'Sign-In'}
                  </Link>
                  { !session?.user ?
                      <Link className="nav-link" href="/">
                          Sign-Up
                      ...
                      :
                      <button type='button' className="nav-link" onClick={() => signOut({callbackUrl: '/'})}>
                          Sign-Out
                      </button>
                  }
              </div>
